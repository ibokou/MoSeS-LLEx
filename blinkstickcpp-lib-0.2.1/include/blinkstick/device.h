...
namespace blinkstick {
   ...
   */
  enum class device_type { unknown, basic, pro, square, strip, nano, flex };

  /**
   * @brief Possible blink stick modes (only valid for Blinkstick Pro).
   */
  enum class mode { unknown = -1, normal, inverse, smart_pixel };
  ...
  public:
    device(std::shared_ptr<hid_device> handle, device_type type);
    /**
     * @brief Sets the LED at the given index and channel to the specified color
     ...
     */
    bool set_colour(int channel, int index, uint8_t red, uint8_t green,
                    uint8_t blue) const;

    bool set_colours(int channel, uint8_t red, uint8_t green,
                     uint8_t blue) const;

    bool set_colours(int channel, const std::vector<colour> &colours) const;
     ...
     * "inverse" (LED values are inverted) and "smart" (LEDs are WS2812 smart
     * LEDs). Note that you'll need to implement a delay after setting the mode
     * before setting the color on the blinkstick device.
     */
     ...
     * @detail This will query the device on the first call, then cache the
     * result so subsequent calls not do query the device
     */
