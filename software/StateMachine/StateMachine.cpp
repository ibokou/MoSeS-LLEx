...
// StateMachine
//----------------------------------------------------------------------------
StateMachine::StateMachine(SemaphoreHandle_t stateMutex, BYTE maxStates,
                           BYTE initialState)
    : m_stateMutex(stateMutex), MAX_STATES(maxStates),
      m_currentState(initialState), m_newState(FALSE), m_eventGenerated(FALSE),
      m_pEventData(NULL) {
...
void StateMachine::ExternalEvent(BYTE newState, const EventData *pData) {
  ...
  if (newState == EVENT_IGNORED) {
    ...
    if (xSemaphoreTake(m_stateMutex, 0) == pdTRUE) {
