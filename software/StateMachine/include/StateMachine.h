...
public:
  ...
  /// evaluates to TRUE.
  virtual BOOL InvokeGuardCondition(StateMachine *sm,
                                    const EventData *data) const = 0;
};
...
public:
  ...
  /// @param[in] sm - A state machine instance.
  /// @param[in] data - The event data.
  virtual void InvokeEntryAction(StateMachine *sm,
                                 const EventData *data) const = 0;
};
...
public:
  ...
  StateMachine(SemaphoreHandle_t stateMutex, BYTE maxStates,
               BYTE initialState = 0);
...
#define GUARD_DECLARE(stateMachine, guardName, eventData)                      \
  ...
  GuardCondition<stateMachine, eventData, &stateMachine::GD_##guardName>       \
      guardName;
...
#define END_TRANSITION_MAP(data)                                               \
  ...
  static_assert((sizeof(TRANSITIONS) / sizeof(BYTE)) == ST_MAX_STATES);
