...
namespace {
  ...
  std::vector<uint8_t> build_control_message(const uint8_t index,
                                             const uint8_t channel, uint8_t red,
                                             uint8_t green, uint8_t blue) {
    // Write to the first LED present
    // this will be the _only_ led for the original blinkstick
    if (index == 0 && channel == 0) {
      return {0x1, red, green, blue};
    }

    // Writing to the other LEDs requires a different payload
    // this changes the write mode (first two bytes) and then
    // assigns the index.
    return {0x5, channel, index, red, green, blue};
  }

  constexpr std::array<uint8_t, MODE_MSG_SIZE>
  build_mode_message(blinkstick::mode mode) {
    return {0x0004, static_cast<uint8_t>(mode)};
  }

  constexpr std::array<uint8_t, COUNT_MSG_SIZE>
  build_count_message(const uint8_t count) {
    return {0x81, count};
  }
  ...
  template <typename T>
  bool send_feature_report(const std::shared_ptr<hid_device> &handle,
                           const T &msg) {
...
namespace blinkstick {
  ...
  device::device(std::shared_ptr<hid_device> handle, device_type type)
      : handle(std::move(handle)), type(type) {}
  ...
  bool device::set_colour(const int channel, const int index, const uint8_t red,
                          const uint8_t green, const uint8_t blue) const {
    if (handle == nullptr) {
      debug("input hid handle is null");
      return false;
    }
  ...
  bool device::set_colours(const int channel, const uint8_t red,
                           const uint8_t green, const uint8_t blue) const {
  ...
  bool device::set_colours(int channel,
  ...
  bool device::off() const { return set_colours(0, 0, 0, 0); }
  ...
  device_type device::get_type() const { return type; }

  bool device::is_valid() const { return handle != nullptr; }
}
