...
static wchar_t *get_usb_string(libusb_device_handle *dev, uint8_t idx) {
  ...
  wchar_t *str = NULL;
    ...
    !defined(NO_ICONV) /* we don't use iconv on Android, or when it is         \
                          explicitly disabled */
  wchar_t wbuf[256];
  /* iconv variables */
  iconv_t ic;
...
/**
  Max length of the result: "000-000.000.000.000.000.000.000:000.000" (39
  chars). 64 is used for simplicity/alignment.
*/
static void get_path(char (*result)[64], libusb_device *dev, int config_number,
  ...
  if (num_ports > 0) {
    int n = snprintf(str, sizeof("000-000"), "%u-%u",
                     libusb_get_bus_number(dev), port_numbers[0]);
    for (uint8_t i = 1; i < num_ports; i++) {
      n += snprintf(&str[n], sizeof(".000"), ".%u", port_numbers[i]);
    }
    n += snprintf(&str[n], sizeof(":000.000"), ":%u.%u", (uint8_t)config_number,
                  (uint8_t)interface_number);
    str[n] = '\0';
  } else {
    /* Likely impossible, but check: USB3.0 specs limit number of ports to 7 and
     * buffer size here is 8 */
...
static char *make_path(libusb_device *dev, int config_number,
                       int interface_number) {
  char str[64];
...
static int
hid_get_report_descriptor_libusb(libusb_device_handle *handle,
                                 int interface_num,
                                 uint16_t expected_report_descriptor_size,
  ...
  */
  int res = libusb_control_transfer(
      ...
      tmp, expected_report_descriptor_size, 5000);
  if (res < 0) {
    LOG("libusb_control_transfer() for getting the HID Report descriptor "
        "failed with %d: %s\n",
 ...
 */
static void fill_device_info_usage(struct hid_device_info *cur_dev,
  ...
  int res = hid_get_report_descriptor_libusb(
      handle, interface_num, expected_report_descriptor_size,
...
static void invasive_fill_device_info_usage(struct hid_device_info *cur_dev,
                                            libusb_device_handle *handle,
                                            int interface_num,
                                            uint16_t report_descriptor_size) {
  int res = 0;
...
#  ifdef DETACH_KERNEL_DRIVER
  ...
  if (res == 1) {
    ...
    if (res < 0)
      LOG("Couldn't detach kernel driver, even though a kernel driver was "
          "attached.\n");
    else
      detached = 1;
  }
...
create_device_info_for_device(libusb_device *device,
                              libusb_device_handle *handle,
                              struct libusb_device_descriptor *desc,
                              int config_number, int interface_num) {
    ...
    if (extra[1] == LIBUSB_DT_HID) { /* bDescriptorType */
      ...
      if (!found_hid_report_descriptor) {
        /* We expect to find exactly 1 HID descriptor (LIBUSB_DT_HID)
           which should contain exactly one HID Report Descriptor metadata
           (LIBUSB_DT_REPORT). */
...
static int is_xbox360(unsigned short vendor_id,
                      const struct libusb_interface_descriptor *intf_desc) {
  ...
  if (intf_desc->bInterfaceClass == LIBUSB_CLASS_VENDOR_SPEC &&
    ...
    size_t i;
    for (i = 0; i < sizeof(supported_vendors) / sizeof(supported_vendors[0]);
         ++i) {
...
static int is_xboxone(unsigned short vendor_id,
                      const struct libusb_interface_descriptor *intf_desc) {
...
static int should_enumerate_interface(
          ...
          if (should_enumerate_interface(dev_vid, intf_desc)) {
            ...
            if (handle) {
              /* There is (a potential) libusb Android backend, in which
                 device descriptor is not accurate up until the device is
                 opened.
                 ...
                 having it here won't do any harm, since reading the device
                 descriptor is as cheap as copy 18 bytes of data. */
              libusb_get_device_descriptor(dev, &desc);
            }
#endif

            tmp = create_device_info_for_device(dev, handle, &desc,
            ...
            if (tmp) {
              ...
              if (handle) {
                uint16_t report_descriptor_size =
                    get_report_descriptor_size_from_interface_descriptors(
                        intf_desc);

                invasive_fill_device_info_usage(tmp, handle,
                                                intf_desc->bInterfaceNumber,
                                                report_descriptor_size);
              }
#endif /* INVASIVE_GET_USAGE */
...
static void *read_thread(void *param) {
  ...
  if (res < 0) {
    LOG("libusb_submit_transfer failed: %d %s. Stopping read_thread from "
        "running\n",
        res, libusb_error_name(res));
    dev->shutdown_thread = 1;
    dev->transfer_loop_finished = 1;
...
static void init_xbox360(libusb_device_handle *device_handle,
  ...
  if ((idVendor == 0x05ac && idProduct == 0x055b) /* Gamesir-G3w */ ||
    ...
    /* The HORIPAD FPS for Nintendo Switch requires this to enable input
       reports. This VID/PID is also shared with other HORI controllers, but
       they all seem to be fine with this as well.
     */
    memset(data, 0, sizeof(data));
    libusb_control_transfer(device_handle, 0xC1, 0x01, 0x100, 0x0, data,
                            sizeof(data), 100);
  }
}

static void init_xboxone(libusb_device_handle *device_handle,
                         unsigned short idVendor, unsigned short idProduct,
                         const struct libusb_config_descriptor *conf_desc) {
    ...
    for (k = 0; k < intf->num_altsetting; k++) {
      ...
      if (intf_desc->bInterfaceClass == LIBUSB_CLASS_VENDOR_SPEC &&
        ...
        if (bSetAlternateSetting) {
          ...
          LOG("Setting alternate setting for VID/PID 0x%x/0x%x interface %d to "
              "%d\n",
              idVendor, idProduct, intf_desc->bInterfaceNumber,
              intf_desc->bAlternateSetting);

          res = libusb_set_interface_alt_setting(device_handle,
                                                 intf_desc->bInterfaceNumber,
                                                 intf_desc->bAlternateSetting);
          if (res < 0) {
...
static int
hidapi_initialize_device(hid_device *dev,
                         const struct libusb_interface_descriptor *intf_desc,
                         const struct libusb_config_descriptor *conf_desc) {
  int i = 0;
  int res = 0;
  ...
  if (res < 0) {
    ...
    if (dev->is_driver_detached) {
      res = libusb_attach_kernel_driver(dev->device_handle,
                                        intf_desc->bInterfaceNumber);
      if (res < 0)
  ...
  dev->report_descriptor_size =
      get_report_descriptor_size_from_interface_descriptors(intf_desc);
  ...
  for (i = 0; i < intf_desc->bNumEndpoints; i++) {
    ...
    int is_output = (ep->bEndpointAddress & LIBUSB_ENDPOINT_DIR_MASK) ==
                    LIBUSB_ENDPOINT_OUT;
    int is_input =
    ...
    for (j = 0; j < conf_desc->bNumInterfaces && !good_open; j++) {
      ...
      for (k = 0; k < intf->num_altsetting && !good_open; k++) {
        ...
        if (should_enumerate_interface(desc.idVendor, intf_desc)) {
          char dev_path[64];
          get_path(&dev_path, usb_dev, conf_desc->bConfigurationValue,
                   intf_desc->bInterfaceNumber);
...
hid_libusb_wrap_sys_device(intptr_t sys_dev, int interface_num) {
/* 0x01000107 is a LIBUSB_API_VERSION for 1.0.23 - version when
 * libusb_wrap_sys_device was introduced */
#if (!defined(HIDAPI_TARGET_LIBUSB_API_VERSION) ||                             \
     HIDAPI_TARGET_LIBUSB_API_VERSION >= 0x01000107) &&                        \
  ...
  res = libusb_get_active_config_descriptor(
      libusb_get_device(dev->device_handle), &conf_desc);
  if (res < 0)
    libusb_get_config_descriptor(libusb_get_device(dev->device_handle), 0,
                                 &conf_desc);
  ...
  if (dev->output_endpoint <= 0) {
    ...
    res = libusb_control_transfer(
        ...
        LIBUSB_REQUEST_TYPE_CLASS | LIBUSB_RECIPIENT_INTERFACE |
            LIBUSB_ENDPOINT_OUT,
        0x09 /*HID Set_Report*/, (2 /*HID output*/ << 8) | report_number,
        dev->interface, (unsigned char *)data, length, 1000 /*timeout millis*/);
...
int HID_API_EXPORT hid_read_timeout(hid_device *dev, unsigned char *data,
	...
	return transferred;
#endif
  /* by initialising this variable right here, GCC gives a compilation
   * warning/error: */
  /* error: variable ‘bytes_read’ might be clobbered by ‘longjmp’ or ‘vfork’
   * [-Werror=clobbered] */
  int bytes_read; /* = -1; */
...
int HID_API_EXPORT hid_send_feature_report(hid_device *dev,
  ...
  res = libusb_control_transfer(
      ...
      LIBUSB_REQUEST_TYPE_CLASS | LIBUSB_RECIPIENT_INTERFACE |
          LIBUSB_ENDPOINT_OUT,
      0x09 /*HID set_report*/, (3 /*HID feature*/ << 8) | report_number,
      dev->interface, (unsigned char *)data, length, 1000 /*timeout millis*/);
...
int HID_API_EXPORT hid_get_feature_report(hid_device *dev, unsigned char *data,
                                          size_t length) {
  int res = -1;
  int skipped_report_id = 0;
  int report_number = data[0];
  ...
  res = libusb_control_transfer(
      ...
      LIBUSB_REQUEST_TYPE_CLASS | LIBUSB_RECIPIENT_INTERFACE |
          LIBUSB_ENDPOINT_IN,
      0x01 /*HID get_report*/, (3 /*HID feature*/ << 8) | report_number,
      dev->interface, (unsigned char *)data, length, 1000 /*timeout millis*/);
...
int HID_API_EXPORT HID_API_CALL hid_get_input_report(hid_device *dev,
  ...
  res = libusb_control_transfer(
      ...
      LIBUSB_REQUEST_TYPE_CLASS | LIBUSB_RECIPIENT_INTERFACE |
          LIBUSB_ENDPOINT_IN,
      0x01 /*HID get_report*/, (1 /*HID Input*/ << 8) | report_number,
      dev->interface, (unsigned char *)data, length, 1000 /*timeout millis*/);
...
int HID_API_EXPORT_CALL hid_get_product_string(hid_device *dev, wchar_t *string,
                                               size_t maxlen) {
...
HID_API_EXPORT struct hid_device_info *HID_API_CALL
hid_get_device_info(hid_device *dev) {
  if (!dev->device_info) {
    ...
    dev->device_info =
        create_device_info_for_device(usb_device, dev->device_handle, &desc,
                                      dev->config_number, dev->interface);
    // device error already set by create_device_info_for_device, if any

    if (dev->device_info) {
      fill_device_info_usage(dev->device_info, dev->device_handle,
                             dev->interface, dev->report_descriptor_size);
    }
  }
...
int HID_API_EXPORT_CALL hid_get_report_descriptor(hid_device *dev,
                                                  unsigned char *buf,
                                                  size_t buf_size) {
  return hid_get_report_descriptor_libusb(dev->device_handle, dev->interface,
                                          dev->report_descriptor_size, buf,
                                          buf_size);
}
